...
- **Traffic puzzles.** Five hand-tuned cities that somebody else built badly, each played as one rush
  hour: a district nobody connected, a single bridge, a crossroads taking the whole town, a lane that
  wanders, and a grid in gridlock. Traffic steps up wave by wave and your money arrives with it, so you
  are rebuilding the city while it is moving. Let the queues stop and the shift is lost.
...
A puzzle is one **shift**: three to five minutes of simulated rush hour on a city you did not build.
The same simulation runs with three rules changed — buildings never grow, decay or get demolished,
there is no income, and utilities are taken as read — so the only thing in play is the traffic.

Every level is a sequence of **waves**. Each one steps the trip rate up and pays an instalment of the
budget the moment it lands, so the money for a problem only ever arrives once the problem is visible.
The opening wave is a city that copes. The second is a city that queues. By the third, a layout nobody
has touched is past saving, and you are rebuilding around traffic that is already moving.

A **gridlock meter** fills while cars sit at a standstill and drains when the roads clear; buildings
with no route to the highway count against it too. Fill it and the shift is over. Survive to the whistle
and you are scored on **trips delivered**, which is where the stars come from. Your best is kept in the
browser.

Bulldozing roads is free, so a wrong turn only costs what you paid to build it. The clock is the
simulation, so pausing stops it as well — pause to think, not to get ahead.

| Level | The problem | Shift | Budget |
| --- | --- | --- | --- |
| First Mile | Two districts with their own streets and no link to the highway | 3:00 | $2,400 |
| One Bridge | Homes one side of the river, every job on the other, one two-lane bridge | 4:20 | $3,800 |
| Four Ways | Every trip in town through a single uncontrolled crossroads | 4:00 | $2,700 |
| The Long Haul | The jobs are at the far end of a lane that was a cart track first | 4:20 | $4,050 |
| Rush Hour | A grid of towers all crossing to the other half at once | 5:00 | $4,600 |
...
- **Scenarios are authored in the highway's frame** (`src/scenarios/`): `along` counts tiles inward
  ...
  roads. The shift itself runs on the main thread (`runtime.ts`) off the worker's per-second report:
  one report is one second, so the clock, the waves and the gridlock meter stay honest at any game
  speed. Waves turn the worker's demand scale up and pay their grant into its money; level cards are
  drawn from the level's own `build()`, so a card cannot show a city the level does not open with.
...
```bash
npm run playtest   # play every level's whole shift, abandoned and to a schedule, and check it holds up
npm run settle     # what each level's city does at each of its wave demands, untouched
...
`npm run playtest` is the one that matters when touching a level or the simulation. It flags any level
an abandoned city can survive, any that no schedule of affordable moves can survive, and any whose
three-star target nothing reached.
