...
export function crossingApproaches(net: Network): Map<number, [number, number]> {
  ...
  for (const node of net.nodes.values()) {
    ...
    for (const seg of arms) {
      const [tx, tz] = direction(seg, node.id), hw = roadHalf(seg);
      let reach = Math.max(...arms.map(s => roadHalf(s))) + 0.24;
      let safe = true;
      for (const other of arms) {
        ...
        reach = Math.max(reach, (roadHalf(other) + hw * Math.abs(dot)) / cross + 0.24);
      }
