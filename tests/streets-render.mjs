...
const [bed] = river.group.children.filter(m => m !== water && !m.isInstancedMesh);
const wp = water.geometry.attributes.position, bp = bed.geometry.attributes.position;
assert.equal(bp.count, wp.count, 'the bed under the water is its own mesh, one vertex per water vertex, with caustics of its own');
for (let i = 0; i < wp.count; i++) assert.ok(wp.getY(i) > bp.getY(i), 'riverbed is below water at every sample');
let bankDisposed = false;
bed.geometry.addEventListener('dispose', () => { bankDisposed = true; });
river.rebuild(terrain);
